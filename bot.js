...
client.on('guildMemberAdd', loz => {
  const mesaj = new Discord.RichEmbed()
    .setDescription(`       <a:suenos_hype:785075309405012009> ** ${loz} ** Hoşgeldin Seni Arazmıda Görmek Ne Güzel!


                            <a:suenos_hype:785075309405012009> Umarım Burada Aradığın Public Ortamı Bulabilirsin!


                            <a:suenos_hype:785075309405012009> Kayıt Olmak İçin **V.Confirmed** Odalarında Beklemelisin!


                            <a:suenos_hype:785075309405012009> <@&781413378236612609> Rolündeki Arkadaşlarımız Seni Kayıt Edecektir.


                            <a:suenos_hype:785075309405012009> Tagımızı Alarak Aramıza Katılabilirsin.


                            <a:suenos_hype:785075309405012009> Şimdiden Aramıza Hoşgeldin ** ${loz} ** `)

   .setImage('https://media.discordapp.net/attachments/757170096375464006/759101741480017970/ezgif.com-crop.gif')

  loz.guild.channels.get(`781463932820258858`).send(mesaj)
});
// HOSGELDIN \\

// BB \\
client.on('guildMemberRemove', loz => {
  const mesaj = new Discord.RichEmbed()
    .setTitle(`GÖRÜŞÜRÜZ`)
    .setDescription(`${loz} Arkana Bakmadan Git!`)
  loz.guild.channels.get(`781463932820258858`).send(mesaj)
});
// BB \\
...
client.on("userUpdate", async(eski, yeni) => {
  ...
  if(!yeni.username.includes("⾚") && client.guilds.get("767667251864141854").members.get(yeni.id).roles.has("781413379964272660")) {
     client.guilds.get("767667251864141854").members.get(yeni.id).removeRole("781413379964272660")
     client.channels.get('767667251864141857').send(`${yeni}, ' ⾚ ' tagını çıkarttığı için <@&781413379964272660> rolü alındı!`)
    }
     if(yeni.username.includes("⾚") && !client.guilds.get("767667251864141854").members.get(yeni.id).roles.has("781413379964272660")) {
      client.channels.get('767667251864141857').send(`${yeni}, ' ⾚ ' tagını aldığı için <@&781413379964272660> rolü verildi!`)
      client.guilds.get("767667251864141854").members.get(yeni.id).addRole("781413379964272660")
     }
  }
  })